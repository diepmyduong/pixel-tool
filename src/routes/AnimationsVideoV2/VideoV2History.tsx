...
interface VideoV2HistoryProps {
  refreshKey: number
  onSelect: (videoBlob: Blob) => void
}

export default function VideoV2History({ refreshKey, onSelect }: VideoV2HistoryProps) {
  const [generations, setGenerations] = useState<RawVideoGeneration[]>([])
  const [urls, setUrls] = useState<Record<string, string>>({})

  useEffect(() => {
    let cancelled = false
    listRawVideoGenerationsByKind('single').then((list) => {
      if (cancelled) return
