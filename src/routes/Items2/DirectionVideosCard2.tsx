...
import type { Item2Sheet, Item2Pose, VideoCellKey } from './useItem2Sheet'
import { POSE_ORDER } from './useItem2Sheet'
...
const POSE_COLOR: Record<Item2Pose, string> = {
  stand: 'blue',
  attacked: 'volcano',
}

interface DirectionVideosCard2Props {
  sheet: Item2Sheet
}

/** Full-width card: one row per direction, each with stand/attacked reference thumbnails and their generated videos. */
export default function DirectionVideosCard2({ sheet }: DirectionVideosCard2Props) {
  const {
    grid,
    sliced,
    rowDirections,
  ...
  const [promptTarget, setPromptTarget] = useState<{ row: number; pose: Item2Pose } | null>(null)
  const [promptDraft, setPromptDraft] = useState('')

  if (!sliced) return null

  function openPromptEditor(row: number, pose: Item2Pose) {
  ...
  return (
    <>
      <Card
        title="Stand / Attacked per direction — click a thumbnail to pick a different cell from the whole sheet"
        extra={
          <Space>
                    ...
                    <div key={pose} style={{ textAlign: 'center', width: 140 }}>
                      <img
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          border: `2px solid ${
                            { blue: '#1677ff', volcano: '#fa541c' }[POSE_COLOR[pose]]
                          }`,
                        }}
                      />
                      <div>
