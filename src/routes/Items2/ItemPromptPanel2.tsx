...
interface ItemPromptPanel2Props {
  name: string
...
export default function ItemPromptPanel2({
  name,
  onNameChange,
  description,
  onDescriptionChange,
...
}: ItemPromptPanel2Props) {
  const prompt = buildItemPrompt2(description, styleTemplate)

  return (
    <Space direction="vertical" style={{ width: '100%' }} size="middle">
        <div>
          <Typography.Text strong>Name</Typography.Text>
          <Input value={name} onChange={(e) => onNameChange(e.target.value)} placeholder="e.g. Wooden Crate" />
        </div>
        ...
        <div>
          ...
          <Typography.Text type="secondary">
            An image of the item sent alongside the prompt as an extra visual reference for generation.
          </Typography.Text>
        </div>
        ...
        <div>
          ...
          <TextArea
            ...
            rows={4}
            placeholder="e.g. a sturdy wooden crate bound with iron bands"
          />
        </div>
