...
import { buildItemPrompt2 } from '../../lib/promptBuilder'
import ItemPromptPanel2 from './ItemPromptPanel2'
import ItemGeneratePanel2 from './ItemGeneratePanel2'
import ItemRawGenerationHistory2 from './ItemRawGenerationHistory2'
import Item2VideoHistoryPage from './Item2VideoHistoryPage'
import Item2SpriteSheetsPage from './Item2SpriteSheetsPage'
import PickSheetCard2 from './PickSheetCard2'
import DirectionVideosCard2 from './DirectionVideosCard2'
import { useItem2Sheet } from './useItem2Sheet'
...
function GenerateTab({ onVideoSaved }: GenerateTabProps) {
  ...
  const prompt = useMemo(() => buildItemPrompt2(description, styleTemplate), [description, styleTemplate])
  ...
  // this page (e.g. ItemRawGenerationHistory2 revokes every URL it handed
  // out whenever its own list refetches), which previously left
  ...
  // useItem2Sheet's grid state (it keys off imageUrl) — upscaling is meant
  // to happen before slicing for exactly this reason: do it first, then
  // align/slice against the sharper source.
  const handleUpscaled = useCallback((blob: Blob) => {
    adoptGeneratedImage(blob, flow2RequestId)
  }, [flow2RequestId])

  const sheet = useItem2Sheet({
    ...
    <div>
      <Row gutter={24}>
        <Col span={10}>
          <Card title="Item">
            ...
            />
            <ItemRawGenerationHistory2 refreshKey={historyKey} onSelect={handleHistorySelect} />
          </Card>
        </Col>
        <Col span={14}>
      ...
      {generatedImageUrl && generatedImageBlob && sheet.sliced && (
        <div style={{ marginTop: 24 }}>
          <DirectionVideosCard2 sheet={sheet} />
        </div>
      )}
    </div>
  )
...
export default function Items2Page() {
  ...
  return (
    <div>
      <Typography.Title level={3}>Item-2</Typography.Title>
      <Tabs
        items={[
          {
            key: 'generate',
