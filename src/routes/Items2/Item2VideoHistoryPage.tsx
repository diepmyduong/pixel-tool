...
import type { Item2VideoEntry, Item2VideoSession } from '../../types'
...
interface Item2VideoHistoryPageProps {
  refreshKey: number
...
function sessionZipFilename(session: Item2VideoSession): string {
  const safeName = (session.name || 'item').replace(/[^a-z0-9_-]+/gi, '_')
  return `${safeName}_videos_${session.id.slice(0, 8)}.zip`
}

function SessionCard({ session, onSessionUpdated }: { session: Item2VideoSession; onSessionUpdated: () => void }) {
...
function Item2VideoHistoryPage({ refreshKey, onSpriteSheetSaved }: Item2VideoHistoryPageProps) {
  const [sessions, setSessions] = useState<Item2VideoSession[]>([])
  const [localRefreshKey, setLocalRefreshKey] = useState(0)

  useEffect(() => {
    let cancelled = false
    listItem2VideoSessions().then((list) => {
      if (cancelled) return
...
export default memo(Item2VideoHistoryPage)
