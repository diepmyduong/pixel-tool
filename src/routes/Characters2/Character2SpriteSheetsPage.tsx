...
interface Character2SpriteSheetsPageProps {
  refreshKey: number
...
interface SheetItem {
  ...
  sessionName: string
  videoIndex: number
  direction: Character2VideoEntry['direction']
  pose: Character2VideoEntry['pose']
  sheetBlob: Blob
  videoBlob: Blob
  createdAt: number
}
 ...
 * separate from Character2VideoHistoryPage (which only shows raw videos) so
 ...
 */
function Character2SpriteSheetsPage({ refreshKey, onSpriteSheetSaved }: Character2SpriteSheetsPageProps) {
  ...
  useEffect(() => {
    let cancelled = false
    listCharacter2VideoSessions().then((sessions) => {
      if (cancelled) return
...
export default memo(Character2SpriteSheetsPage)
