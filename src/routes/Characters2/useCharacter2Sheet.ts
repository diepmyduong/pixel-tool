...
import { CHAR2_GRID_ROWS, CHARACTER2_POSE_ORDER } from '../../lib/grid'

export type { Character2Pose }
export const POSE_ORDER = CHARACTER2_POSE_ORDER

export type VideoCellKey = `${number}-${Character2Pose}`
...
// The generation prompt (buildCharacterPrompt2) asks for a strict 3-column
// sheet (stand/run/attack), but the model often draws each pose across 2
// columns instead of 1, so the picker's starting grid is seeded wider than
// the prompt to match what actually comes back — the row/column controls
// still let it be adjusted either way.
const INITIAL_SLICE_COLS = 6
...
interface UseCharacter2SheetArgs {
  imageUrl: string
  imageBlob: Blob
  name: string
  description: string
...
/**
 * All state/logic for the Characters-2 "Pick this sheet" + "Stand/Run/Attack
 * per direction" screens, lifted out of the components so the two card UIs
 * can be placed independently in the page layout (the direction-videos card
 * needs to span the full page width, outside the narrow column the sheet
 * picker lives in) while still sharing one source of truth.
 */
export function useCharacter2Sheet({ imageUrl, imageBlob, name, description, onVideoSaved }: UseCharacter2SheetArgs) {
  ...
  const [rowPoseRef, setRowPoseRef] = useState<Record<number, Partial<Record<Character2Pose, ImageRef>>>>({})
  ...
  const [pickerTarget, setPickerTarget] = useState<{ row: number; pose: Character2Pose } | null>(null)
  ...
  // Per-(row, pose) prompt override — absent means "use buildCharacter2VideoPrompt's
  ...
  function getEffectivePrompt(row: number, pose: Character2Pose): string {
    const key: VideoCellKey = `${row}-${pose}`
    const direction = rowDirections[row]
    return customPrompts[key] ?? (direction ? buildCharacter2VideoPrompt(pose, direction, description) : '')
  }

  function setCustomPrompt(row: number, pose: Character2Pose, prompt: string) {
    const key: VideoCellKey = `${row}-${pose}`
    setCustomPrompts((prev) => ({ ...prev, [key]: prompt }))
  }

  function resetCustomPrompt(row: number, pose: Character2Pose) {
    const key: VideoCellKey = `${row}-${pose}`
  ...
  function canvasForPose(row: number, pose: Character2Pose): HTMLCanvasElement | undefined {
    return canvasForRef(rowPoseRef[row]?.[pose])
  }

  function setPoseRef(row: number, pose: Character2Pose, ref: ImageRef) {
  ...
  async function handleSlice() {
    ...
    // attack always defaults to the LAST column — the AI reliably puts the
    // attack pose at the end of the row even when it draws more or fewer
    // columns than expected, while spreading the remaining poses evenly
    // across whatever's left in between avoids stand/run defaulting to the
...
export type Character2Sheet = ReturnType<typeof useCharacter2Sheet>
